...
new_array=struct.pack("<QQQ",ENTRY0_VA,entry1_va,0)

# ---- new relocs: entry1 pointers (8) + array slots (2) ----
new_relocs=[]
for delta,add in ent0_relocs.items():
    new_relocs.append([entry1_va+delta,RELATIVE,add])
new_relocs.append([array_va+0x00,RELATIVE,ENTRY0_VA])
new_relocs.append([array_va+0x08,RELATIVE,entry1_va])
assert len(new_relocs)==10
